...
| API Docs | Scramble (spec generation) + Swagger UI (interactive UI) |
| Container | Docker + OPcache for local performance |
...
The interactive docs are at `http://api.budget.test/docs/api` (Swagger UI). The spec is auto-generated by Scramble from route definitions and docblocks.

**Authorizing in the UI:**
...
All protected endpoints will now include `Authorization: Bearer <token>` automatically. The token is saved in `localStorage` and restored on page reload.
...
API is available at `http://api.budget.test/api`.

### OPcache

OPcache is enabled in the Docker image (`opcache.ini`) with `validate_timestamps=1` and `revalidate_freq=0` — PHP picks up file changes on every request so the dev workflow is unaffected, but compiled opcodes are served from memory rather than re-read from the bind-mounted Windows filesystem on each request.
